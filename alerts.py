...
def send_price_drop_alert_telegram(chat_id: str, product_name: str, old_price: float,
                                    new_price: float, drop_percent: float, product_url: str) -> bool:
...
def send_buy_now_alert_telegram(chat_id: str, product_name: str, current_price: float,
                                 prediction: Dict, product_url: str) -> bool:
...
def send_target_price_alert_telegram(chat_id: str, product_name: str, current_price: float,
                                      target_price: float, product_url: str) -> bool:
...
def send_price_drop_alert_email(user_email: str, product_data: Dict, old_price: float,
...
def send_buy_now_alert_email(user_email: str, product_data: Dict, current_price: float,
...
def dispatch_price_alert(user: Dict, track: Dict, product_data: Dict,
                         old_price: float, new_price: float,
                         drop_percent: float, prediction: Dict = None,
                         target_reached: bool = False) -> List[str]:
    ...
    if track.get("alert_telegram", False) and telegram_chat_id:
        if target_reached and prediction:
            ok = send_target_price_alert_telegram(telegram_chat_id, product_data["name"],
                                                  new_price, track.get("target_price", 0),
                                                  product_data.get("url", ""))
        elif prediction:
            ok = send_buy_now_alert_telegram(telegram_chat_id, product_data["name"],
                                             new_price, prediction, product_data.get("url", ""))
        else:
            ok = send_price_drop_alert_telegram(telegram_chat_id, product_data["name"],
                                                old_price, new_price, drop_percent,
                                                product_data.get("url", ""))
        if ok:
